...
void* __fastcall Redirected_Copy_User_Command(void* Unknown_Parameter_1, void* Unknown_Parameter_2, User_Command_Structure* User_Command)
{
	if (Freeze_Controlled_Creature == 0)
	{
		...
		if (*(void**)_AddressOfReturnAddress() == Copy_User_Command_Create_Move_Return_Location)
		{
			...
			{
				static unsigned __int32 Recorded_Commands_Maximum_Elements_Amount = Recorded_Route.max_size();

				if (Recorded_Commands.size() != Recorded_Commands_Maximum_Elements_Amount)
				{
					Recorded_Commands.push_back(*(User_Command_Structure*)User_Command);
			...
			else
			{
				...
				{
					if (Recorder_User_Comamand_Number != Recorded_Commands.size())
					{
						User_Command_Structure* Recorded_Command = &Recorded_Commands.at(Recorder_User_Comamand_Number);

						User_Command->View_Angles[0] = Recorded_Command->View_Angles[0];

						User_Command->View_Angles[1] = Recorded_Command->View_Angles[1];

						User_Command->View_Angles[2] = Recorded_Command->View_Angles[2];
						...
						User_Command->Forward_Move = Recorded_Command->Forward_Move;

						User_Command->Side_Move = Recorded_Command->Side_Move;

						User_Command->Up_Move = Recorded_Command->Up_Move;

						User_Command->Buttons_State = Recorded_Command->Buttons_State;

						User_Command->Impulse = Recorded_Command->Impulse;

						User_Command->Mouse_Difference_X = Recorded_Command->Mouse_Difference_X;

						User_Command->Mouse_Difference_Y = Recorded_Command->Mouse_Difference_Y;
